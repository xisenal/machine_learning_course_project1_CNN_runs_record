...
def train_kfold(data_root='data/CUB_200_2011', epochs=20, batch_size=16, lr=1e-3, num_folds=10):
    ...
    # 初始化 wandb 运行（如果可用），默认离线以避免卡在上传
    wandb_run = None
    if _WANDB_AVAILABLE:
        try:
            wandb_mode = os.getenv('WANDB_MODE', 'offline')  # 如需在线，设置 WANDB_MODE=online
            wandb_settings = wandb.Settings(start_method='thread')  # Windows 兼容
            wandb_entity = os.getenv('WANDB_ENTITY')
            wandb_project = os.getenv('WANDB_PROJECT', 'cnn-experiments')
            if not wandb_entity:
                print("[INFO] WANDB_ENTITY 未设置，将使用默认账号空间；如遇 401，请设置 WANDB_ENTITY")
    ...
    for fold in range(num_folds):
        ...
        best_val_acc = 0.0
        patience, bad_epochs = 19, 0  # 提高早停容忍度，尽量跑满 20 个 epoch 观察曲线
        fold_metrics = []
    ...
    if wandb_run is not None:
        try:
            ...
            # 无论是否异常，确保关闭，离线模式不会阻塞上传
            wandb_run.finish()
    # 如果是离线模式且用户启用自动同步，则在训练结束后尝试同步到云端
    try:
        if os.getenv('WANDB_MODE', 'offline') == 'offline' and os.getenv('WANDB_AUTO_SYNC', '0') == '1':
            wandb_entity = os.getenv('WANDB_ENTITY')
            wandb_project = os.getenv('WANDB_PROJECT', 'cnn-experiments')
            ...
            cmd += ['--include-offline', '--include-synced', 'wandb/']
            subprocess.run(cmd, check=False)
    except Exception as e:
        print(f"[WARN] wandb offline sync failed: {e}")

def wandb_preflight():
    """简单检查 W&B 登录状态与 entity/project 配置，并给出建议命令。"""
    print("\n===== W&B Preflight Check =====")
    if not _WANDB_AVAILABLE:
        print("[ERROR] wandb 未安装，请先安装：pip install wandb")
        return
    try:
        # 检查 API Key
        from wandb.apis import InternalApi  # type: ignore
        api = InternalApi()
        key = api.api_key
        print(f"[INFO] API Key: {'已配置' if key else '未配置'}")
        if not key:
            print("[HINT] 运行 `python -m wandb login` 或设置 $env:WANDB_API_KEY 后登录")
        # 环境参数
        wandb_entity = os.getenv('WANDB_ENTITY')
        wandb_project = os.getenv('WANDB_PROJECT', 'cnn-experiments')
        wandb_mode = os.getenv('WANDB_MODE', 'offline')
        print(f"[INFO] MODE={wandb_mode} | ENTITY={wandb_entity or '(默认)'} | PROJECT={wandb_project}")
        # 给出同步建议
        cmd = [sys.executable, '-m', 'wandb', 'sync']
        if wandb_entity:
            cmd += ['--entity', wandb_entity]
...
if __name__ == "__main__":
    # 支持快速预检：python main.py wandb_check
    if len(sys.argv) > 1 and sys.argv[1].lower() == 'wandb_check':
        wandb_preflight()
    else:
